...
public class GetPeaksPerScanCountChartDataServiceAction  extends Action {

    
    private static final Logger log = Logger.getLogger(GetPeaksPerScanCountChartDataServiceAction.class.getName());
		...
		try {
			...
			for ( int index = 0; index < experimentIDsStringSplit.length; index++ ) {
				...
				try {
					...
					experimentIds[ index ] = Integer.parseInt( experimentIdString );
				} catch ( Exception e ) {
			...
			for ( int index = 0; index < experimentIds.length; index++ ) {
				...
				String peakCountChartData = null;
				
				try {
				
		            peakCountChartData = QC_Plot_PeaksPerScanPerExperiment_Plotter.getPeaksPerScanPerExperimentPlot(experimentId);
				
				} catch ( Exception ex ) {
					
					log.error( "GetPeaksPerScanCountChartDataServiceAction:  Exception getting peaksPerScanChartData for experimentId: " + experimentId, ex );
				...
				if ( peakCountChartData != null ) {
					...
					outputDataSB.append( peakCountChartData );
				}
			}
			...
			try {
				...
				log.error( "GetPeaksPerScanCountChartDataServiceAction:  Exception writing response.  experimentIDs: " + experimentIDsString, ex );
			...
			log.error( "GetPeaksPerScanCountChartDataServiceAction:  Sending '500' response.  experimentIDs: " + experimentIDsString, ex );
