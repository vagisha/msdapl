...
//   experimentDetailsPeakCountChart.js
...
function createAllInitialDisplayPeakCountCharts() {


	//	delay to disconnect from page loading Javascript execution
	setTimeout(function(){

		CreatePeakCountCharts.createAllInitialDisplayPeakCountCharts();
...
var CreatePeakCountChartsClass = function () {
	...
	this.SERVICE_URLS = {
			...
			GET_CHART_DATA_FOR_EXPERIMENT_IDS: this.webAppContextPath_WebApp_Wide_Val + "/getPeaksPerScanCountChartDataService.do"
	};
...
//Create the PeakCount Chart for the passed in experiment id

CreatePeakCountChartsClass.prototype.createDisplayPeakCountChartForExperimentId = function( experimentId ) {
...
//  Create the PeakCount Chart for all the experiments on the page that loaded successfully

CreatePeakCountChartsClass.prototype.createAllInitialDisplayPeakCountCharts = function(  ) {
...
//		if ( uploadSuccess === "true" && experiment_has_full_information === "true" ) {
		...
		if ( uploadSuccess === "true" ) {
			...
			var $chartData = $experimentDetailsDiv.find(".experiment_peak_count_chart_data_jq");
			...
			if ( chartDataString === "" ) {
				...
				objectThis.createPeakCountChartActual( $experimentDetailsDiv, chartDataFromServer, experimentIdString );
			}
		}
	});
...
CreatePeakCountChartsClass.prototype.getChartDataViaAjax = function ( experimentIdsForGetViaAjax, $experiment_details_outer_div_list, experimentIdIndex ) {
...
CreatePeakCountChartsClass.prototype.processChartDataFromAjax = function ( param ) {
...
//	var context = param.context;
	...
	for ( var index = 0; index < chartDataArrayFromServer.length; index++ ) {
		...
		objectThis.createPeakCountChartActual( $experimentDetailsDiv, chartDataFromServer, chartDataFromServer.experimentId );
	}
};

/////////////////////////////////////

//  Create the PeakCount Chart for one experiment on the page

CreatePeakCountChartsClass.prototype.createPeakCountChartActual = function ( $experimentDetailsDiv, chartDataFromServer, experimentIdString ) {
...
//	chartArea:{left:10,top:20,width:"100%",height:"100%"}, // left and top options will define the amount of padding from left and top
	...
	var chartThumbnailReadyHandler = function( paramNotProvided) {
		...
		var $experiment_peak_count_chart_outer_div_jq = 
			$experimentDetailsDiv.find(".experiment_peak_count_chart_outer_div_jq");

		$experiment_peak_count_chart_outer_div_jq.show();
	...
	var $experiment_peak_count_chart_click_for_full_size_jq = 
		$experimentDetailsDiv.find(".experiment_peak_count_chart_click_for_full_size_jq");

	$experiment_peak_count_chart_click_for_full_size_jq.click( function() {
		...
		var $divInsideFullSizeChartDiv = $experiment_peak_count_chart_full_size_jq.find("div");
		
		//  Detect if full size chart already created
		if ( $divInsideFullSizeChartDiv.length === 0 ) {
		
			$divInsideFullSizeChartDiv = $("<div actual_full_size_chart_holder='true'></div>").appendTo( $experiment_peak_count_chart_full_size_jq );
			...
			var optionsFullsize = {
					...
					//  X axis label below chart
					hAxis: { title: 'Peak Count', titleTextStyle: {color: 'black'}
					},  
		...
		$chartThumbnailDiv.colorbox(
				{transition:"none", 
					...
					href:$experiment_peak_count_chart_full_size_jq  //  Use HTML element specified in href to display in colorbox
				});
	});
};
...
CreatePeakCountChartsClass.prototype.get$experimentDetailsDivFromExperimentId = function( experimentId ) {
...
CreatePeakCountChartsClass.prototype.isThumbnailAlreadyCreated = function( $experimentDetailsDiv ) {
...
//  Declare CreatePeakCountCharts namespace by creating a variable 

var CreatePeakCountCharts = new CreatePeakCountChartsClass(); 


//assign to window
window.CreatePeakCountCharts = CreatePeakCountCharts;
