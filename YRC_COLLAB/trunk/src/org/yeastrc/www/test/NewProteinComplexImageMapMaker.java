 ...
 */
public class NewProteinComplexImageMapMaker {

	private NewProteinComplexImageMapMaker() { }
	public static NewProteinComplexImageMapMaker getInstance() {
		return new NewProteinComplexImageMapMaker();
	}
	...
	public void createProteinComplexImageMap( NRProtein protein, int publication, Graph2D graph, LinkMap linkMap ) throws Exception {
		...
		String sql = "SELECT DISTINCT proteinID FROM tblComplexMembers WHERE complexID IN ( SELECT complexID FROM tblComplexMembers AS a INNER JOIN tblComplex AS b ON a.complexID = b.id WHERE a.proteinID = " + focalProtein.getId() + " AND b.publicationID = " + publication + ")";
