 ...
 */
public class ProteinComplexThumbnailMaker {

	private ProteinComplexThumbnailMaker() { }
	public static ProteinComplexThumbnailMaker getInstance() {
		return new ProteinComplexThumbnailMaker();
	}
	...
	public BufferedImage getThumbnail( NRProtein focalProtein, int publicationID ) throws Exception {
		...
		String sql = "SELECT complexID FROM tblComplexMembers AS A INNER JOIN tblComplex AS B ON A.complexID = b.id WHERE A.proteinID = " + focalProtein.getId() + " AND B.publicationID = " + publicationID; 
