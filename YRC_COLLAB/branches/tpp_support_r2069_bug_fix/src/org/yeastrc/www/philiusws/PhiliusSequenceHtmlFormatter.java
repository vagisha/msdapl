...
public class PhiliusSequenceHtmlFormatter {
	...
	public String format(PhiliusSequenceAnnotationWS psa, Set<String> peptideSequences) throws Exception {

		if ( psa.getSegments() == null || psa.getSegments().size() < 1 )
			return null;
		
		String sequence = psa.getSequence();
		...
		List<PhiliusSegmentWS> segments = psa.getSegments();
		...
		for (int i = 0; i < residues.length; i++ ) {
			...
			for ( PhiliusSegmentWS segment :segments ) {
				if ( segment.getStart() == counter ) {
					isStart = true;
					StringBuilder tooltip = new StringBuilder( "Segment Type: " + segment.getTypeString() + "<br>" );
			...
			if(coveredResidues[i] == 1)
				retStr += "<span style=\"color:black\"><B><U>";
			retStr += residues[i];
			if(coveredResidues[i] == 1)
				retStr += "</U></B></span>";
	...
	public String getPhiliusLegend() {
		...
		buf.append("<b>Legend: </b><span "+getStyleForSegmentType(3)+">Transmembrane Helix</span>, ");
		buf.append("<span "+getStyleForSegmentType(1)+">Non-Cytoplasmic</span>, ");
		buf.append("<span "+getStyleForSegmentType(2)+">Cytoplasmic</span>, ");
		buf.append("<span "+getStyleForSegmentType(0)+">Signal Peptide</span>");
		return buf.toString();
	}
	
	private String getStyleForSegmentType(int type) {
		switch (type) {
		case 0:
