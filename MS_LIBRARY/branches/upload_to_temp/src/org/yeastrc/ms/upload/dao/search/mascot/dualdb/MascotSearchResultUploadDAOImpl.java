 ...
 * 1. MascotSearchResult
 */
public class MascotSearchResultUploadDAOImpl implements MascotSearchResultUploadDAO, TableCopier {

    private static final Logger log = Logger.getLogger(MascotSearchResultUploadDAOImpl.class.getName());
    
    private final MascotSearchResultUploadDAO resultDao;
     ...
     */
    public MascotSearchResultUploadDAOImpl(MascotSearchResultUploadDAO resultDao, boolean useTempTable) {
    ...
    public void saveAllMascotResultData(List<MascotResultDataWId> resultDataList) {
        resultDao.saveAllMascotResultData(resultDataList);
    }
    ...
    public void copyToMainTable() throws TableCopyException {
        ...
        if(useTempTable) {
            ...
            copier.copyToMainTableFromFile("MascotSearchResult", true); // disable keys on main database table before copying 
        }
        else {
    ...
    public boolean checkBeforeCopy() throws TableCopyException {
        TableCopyUtil copier = TableCopyUtil.getInstance();
        if(!copier.checkColumnValues("MascotSearchResult", "resultID"))
            return false;
        return true;
    }
}
